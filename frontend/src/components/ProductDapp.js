...
// transaction.
export class ProductDapp extends React.Component {
  constructor(props) {
    super(props);
          ...
          <div className="col-12">
            ...
            */}
            {this.state.balance.eq(0) && (
              <NotManufacturer selectedAddress={this.state.selectedAddress} />
            )}
  ...
  async _initializeEthers() {
    ...
    this._manufacturer = new ethers.Contract(
      contractAddress.Manufacturer,
      ManufacturerArtifact.abi,
      this._provider.getSigner(0)
    );

    this._productNft = new ethers.Contract(
        contractAddress.ProductNFT,
        ProductNFT.abi,
        this._provider.getSigner(0)
      );
  }
  ...
  async _getTokenData() {
    ...
    this.setState({ tokenData: { name: 'Manufacturer', symbol: "MNT" } });
  }
  ...
  // send a transaction.
  async _mintProduct(brand, serial) {
    ...
    try {
      ...
      const tx = await this._productNft.mintProduct(brand, serial);
